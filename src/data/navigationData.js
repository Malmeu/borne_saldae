  ...
  for (let i = 0; i < pathNodeIds.length - 1; i++) {
    ...
    if (edge) {
      if (edge.type === "elevator" && u.floor !== v.floor) {
        instructionsFR.push(`Prenez l'ascenseur principal jusqu'au ${v.floor === 0 ? "Rez-de-chaussée" : v.floor === 1 ? "1er étage" : "2e étage"}.`);
        instructionsAR.push(`خذ المصعد الرئيسي إلى ${v.floor === 0 ? "الطابق الأرضي" : v.floor === 1 ? "الطابق الأول" : "الطابق الثاني"}.`);
